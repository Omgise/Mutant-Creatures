...
public class RenderEnderBlock extends Render {
   ...
   public void doRender(Entity entity, double d, double d1, double d2, float f, float f1) {
      ...
      int id = enderBlock.getBlockID();
      int data = enderBlock.getBlockData();
      Block block = Block.getBlockById(id);
      GL11.glEnable(32826);
      GL11.glPushMatrix();
      GL11.glTranslatef((float)d, (float)d1, (float)d2);
      float tick = (float)enderBlock.ticksExisted + f1;
      GL11.glRotatef(45.0F, 0.0F, 1.0F, 0.0F);
      GL11.glRotatef(((float)enderBlock.ticksExisted + f1) * 20.0F, 1.0F, 0.0F, 0.0F);
      GL11.glRotatef(((float)enderBlock.ticksExisted + f1) * 12.0F, 0.0F, 0.0F, -1.0F);
      float scale = 0.75F;
      GL11.glScalef(-scale, -scale, scale);
      this.bindEntityTexture(enderBlock);
